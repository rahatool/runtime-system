...
void NetHandle::Close() {
	...
	// The async NET_Close primitive will remove from map.
	uv_close(handle, [](uv_handle_t* h){
		// The NetHandle itself is deleted by the shared_ptr in HandleStore
		delete (uv_tcp_t*)h;
	});
}
...
void OnWrite(uv_write_t* req, int status) {
	...
	else context->Resume(v8::Integer::New(context->fiber->isolate(), context->buf.len));
	delete context;
}
...
void OnRead(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf) {
	...
	if (context == nullptr) { delete[] buf->base; return; }
	...
	if (nread < 0) {
		...
		context->data.assign(buf->base, nread);
		context->Resume(v8::Integer::New(context->fiber->isolate(), nread));
	}
	delete[] buf->base;
	delete context;
}
...
// --- JS Primitives ---
void NET_TcpListen(const v8::FunctionCallbackInfo<v8::Value>& args) {
...
void NET_TcpAccept(const v8::FunctionCallbackInfo<v8::Value>& args) {
...
void NET_TcpConnect(const v8::FunctionCallbackInfo<v8::Value>& args) {
	v8::Isolate* isolate = args.GetIsolate();
	v8::String::Utf8Value host_str(isolate, args[0]);
	int port = args[1]->Int32Value(isolate->GetCurrentContext()).ToChecked();
...
void NET_Read(const v8::FunctionCallbackInfo<v8::Value>& args) {
	v8::Isolate* isolate = args.GetIsolate();
	uint64_t id = args[0].As<v8::BigInt>()->Uint64Value();
	auto wrap = HandleStore::Get<NetHandle>(id);
	...
	v8::Local<v8::Value> result = Fiber::get_current()->resume_value.Get(isolate);
	if(result->IsNativeError()) { isolate->ThrowException(result); return; }
	if (result->IsNull()) { args.GetReturnValue().Set(v8::Null(isolate)); return; } // EOF
	
	args.GetReturnValue().Set(
		v8::String::NewFromUtf8(isolate, context->data.c_str(), v8::NewStringType::kNormal, context->data.length()).ToLocalChecked()
	);
}
void NET_Write(const v8::FunctionCallbackInfo<v8::Value>& args) {
	v8::Isolate* isolate = args.GetIsolate();
	uint64_t id = args[0].As<v8::BigInt>()->Uint64Value();
	v8::String::Utf8Value data_str(isolate, args[1]);
	
	auto wrap = HandleStore::Get<NetHandle>(id);
	if (!wrap) { Throw(isolate, "Invalid socket handle"); return; }

	WriteContext* context = new WriteContext(Fiber::get_current(), *data_str, data_str.length());
...
void NET_Close(const v8::FunctionCallbackInfo<v8::Value>& args) {
	uint64_t id = args[0].As<v8::BigInt>()->Uint64Value();
	auto wrap = HandleStore::Get<NetHandle>(id);
	if (!wrap) { Throw(args.GetIsolate(), "Invalid socket handle"); return; }
	wrap->Close(); // This calls uv_close
...
void InitializeNet(v8::Isolate* isolate, v8::Local<v8::Object> exports) {
	...
	v8::Local<v8::Object> net_obj = v8::Object::New(isolate);
	SET_METHOD(net_obj, "listen", NET_TcpListen);
	SET_METHOD(net_obj, "accept", NET_TcpAccept);
	SET_METHOD(net_obj, "connect", NET_TcpConnect);
	SET_METHOD(net_obj, "read", NET_Read);
	SET_METHOD(net_obj, "write", NET_Write);
	SET_METHOD(net_obj, "close", NET_Close);
	SET_METHOD(net_obj, "poll", NET_Poll);
	exports->Set(context, v8::String::NewFromUtf8(isolate, "net").ToLocalChecked(), net_obj).Check();
}